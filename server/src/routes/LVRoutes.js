// server/src/routes/LVRoutes.js
const express = require('express');
const router = express.Router();
const { processLVWithSummary } = require('../signals/dataProcessor/LV');
...
router.get('/:symbol/:granularity', async (req, res) => {
  try {
    ...
    const lvResults = processLVWithSummary(symbol.toUpperCase(), granularity, ohlcData);
    res.json(lvResults);
  } catch (error) {
    console.error('Failed to process LV data:', error);
    res.status(500).json({ message: 'Error processing LV data.' });
  }
});
